    ...
    <div className="px-6 py-8 lg:px-8 lg:py-24">
      <div className="mx-auto max-w-3xl text-base leading-7 text-gray-700">
        ...
        <div className="flex justify-between w-full flex-nowrap">
          {prev && (
            <span className="flex-1 flex-grow py-8 rounded-xl mt-8 outline outline-gray-200 mr-4 lg:mr-10 ">
              <Link href={`/review/${prev.slug}`} className="flex text-gray-300 hover:text-gray-700 ml-5">
                <span className="text-sm font-medium">Previous Review</span>
              </Link>
              <Link href={`/review/${prev.slug}`} className="block mt-2 text-md font-semibold text-gray-900 hover:text-gray-700 ml-5 mr-5">{prev.title}</Link>
            </span>
          )}
          {next && (
            <span className="flex-1 flex-grow py-8 text-right rounded-xl outline outline-gray-200 mt-8 ml-4 lg:ml-10">
              <Link href={`/review/${next.slug}`} className="flex justify-end text-gray-300 hover:text-gray-700 mr-5">
                <span className="text-sm font-medium">Next Review</span>
              </Link>
              <Link href={`/review/${next.slug}`} className="block mt-2 text-md font-semibold text-gray-900 hover:text-gray-700 mr-5 ml-5">{next.title}</Link>
            </span>
          )}
        </div>
      </div>
